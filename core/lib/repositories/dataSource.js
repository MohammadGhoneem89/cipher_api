...
function findPageAndCount(payload) {
  ...
  return Promise.all([
    dataSource
      ...
      .select('dataSourceName createdBy type sourceFunction filePath updatedAt createdAt createdID')
...
function getServiceList() {
  ...
  return dataSource
    ...
    .populate({ path: 'dipatcher.endpointName' })
    .populate({ path: 'dipatcher.templateName' })
    .sort('dataSourceName')
    .lean(true)
    .exec();
}
