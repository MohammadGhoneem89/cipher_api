...
let ImageUpload = function (data, UUID, params, userID, source, context, imageUpload_CB) {
  ...
  let imageData = {};
  console.log(ext);
  let publicImageUploadExt = config.get('publicImageUploadExt', ['jpg', 'png', 'gif']);

  if (publicImageUploadExt.indexOf(ext) == -1) {
    response.responseMessage.data.message.errorDescription = "Extention Not Allowed";
    return imageUpload_CB(response);
  }
