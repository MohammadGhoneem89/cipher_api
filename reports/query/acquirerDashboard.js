...
function acquirerDashboardSummary(body) {
  ...
  if(body.filter && body.filter.length>0){
     viewUrl = '_design/acquirerWorkboard/_view/acquirerWorkboard';
     viewUrlExp = '_design/acquirerWorkboard/_view/acquirerWorkboard';
     option = { startkey: [body.filter,fromDate], endkey: [body.filter,toDate], reduce: true, group_level: 2, group: true }; // eslint-disable-line camelcase
     optionExp = { startkey: [body.filter,fromDateExp], endkey: [body.filter,toDateExp], reduce: true, group_level: 2, group: true }; // eslint-disable-line camelcase
  } else {
     viewUrl = '_design/acquirerWorkboardByTimestamp/_view/acquirerWorkboardByTimestamp';
     viewUrlExp = '_design/acquirerWorkboardByTimestamp/_view/acquirerWorkboardByTimestamp';
  ...
  let viewUrlCommission="_design/lastCommissionDate/_view/lastCommissionDate";
  let optionCommission = { limit : 1000000, reduce: true, group_level: 1, group: true };
  ...
  return selectWithView(channel, viewUrl, option ).then((response) => {
    ...
    return selectWithView(channelExp, viewUrlExp, optionExp ).then((responseExp) => {
        responseArr.push(responseExp);
         return selectWithView(channelExp, viewUrlCommission, optionCommission ).then((responseCommission) => {
            responseArr.push(responseCommission);
            return responseArr;
        });
    });
  });
  //const option = { limit:10000, reduce: true }; // eslint-disable-line camelcase
}

module.exports = acquirerDashboardSummary;
