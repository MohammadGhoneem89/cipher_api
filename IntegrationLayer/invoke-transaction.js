	...
	return helper.getRegisteredUsers(username, org).then((user) => {
		...
		if (all_good) {
			...
			return Promise.all([sendPromise].concat(eventPromises)).then((results) => {
				...
				logger.error(
					'Failed to send transaction and get notifications.js within the timeout period.'
				);
				return 'Failed to send transaction and get notifications.js within the timeout period.';
			});
		} else {
			logger.error(
		...
		let resp = {
				success: true,
				message: JSON.stringify('Failed to send proposal due to error: ' + err.stack ? err.stack : err)
		};
		...
		if (response.status === 'SUCCESS') {
			...
			var Result=tx_id.getTransactionID();
			let resp = {
				success: true,
		...
		let resp = {
				success: false,
				message: 'Failed to send transaction due to error: ' + err.stack ? err.stack : err
			};
		return resp;
	});
};
