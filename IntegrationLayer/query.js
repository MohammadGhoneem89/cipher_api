...
var queryChaincode = function(peer, channelName, chaincodeName, args, fcn, username, org) {
	...
	return helper.getRegisteredUsers(username, org).then((user) => {
		...
		var result=channel.queryByChaincode(request, target);
		
	return result;
	}, (err) => {
		let response = {
				success: false,
				message: util.format('Failed to get submitter \'%s\'. Error: %s', username,err.stack ? err.stack : err)
			};
		...
		if (response_payloads) {
			...
			if(response_payloads[i].toString('utf8').indexOf('Error') !== -1){
				successFlag=false;
			}
		...
		let response = {
				success: false,
				message:  util.format('Failed to send query due to error: %s', err.stack ? err.stack : err)
			};
		return response;
		...
		let response = {
				success: false,
				message: util.format('Failed to end to end test with error: %s', err.stack ? err.stack : err)
			};
		return response;
...
var getBlockByNumber = function(peer, blockNumber, username, org) {
	...
	return helper.getRegisteredUsers(username, org).then((member) => {
		...
		let response = {
				success: true,
				message: channel.queryBlock(parseInt(blockNumber), target)
		};
		return response;
	}, (err) => {
		logger.info('Failed to get submitter "' + username + '"');
		...
		let response = {
				success: false,
				message: util.format('Failed to query with error: %s', err.stack ? err.stack : err)
		};
		return response;
	});
};
