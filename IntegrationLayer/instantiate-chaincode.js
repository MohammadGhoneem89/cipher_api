...
var instantiateChaincode = function(channelName, chaincodeName, chaincodeVersion, functionName, args, username, org) {
    ...
    hfc.setConfigSetting('request-timeout', 12000000);
	...
	return helper.getOrgAdmin(org).then((user) => {
		...
		if (all_good) {
			...
			return Promise.all([sendPromise].concat([txPromise])).then((results) => {
				...
				logger.error(
					util.format('Failed to send instantiate transaction and get notifications.js.js within the timeout period. %s', err)
				);
				return 'Failed to send instantiate transaction and get notifications.js.js within the timeout period.';
			});
		} else {
			logger.error(
		...
		if (response.status === 'SUCCESS') {
			...
			let respx= {
				success: true,
				message: 'Chaincode Instantiateion is SUCCESS'
			};
			...
			logger.error(util.format('Failed to order the transaction. Error code:  %s ',response.status));
		...
		let respx = {
				success: false,
				message: util.format('Failed to send instantiate due to error:  %s ',err.stack ? err.stack : err)
			};
			return respx;
