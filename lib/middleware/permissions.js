...
module.exports = (req, res, next) => {
  ...
  console.log("Checking for username in request.");
  ...
  if (header.username) {
    ...
    if (global.TLB && global.TLB[header.username] != undefined) {
      ...
      console.log("Cache MISS Detected, Querying DataBase!!");
    }
  }
  else {
  ...
  console.log("cheching for illeagal characters.");
  ...
  function validateToken(token) {
    return tokenLookupRepo.findOne({ token: token })
      .then((res) => {
        ...
        if (diff > config.get('tokenExp') || diff < 0) {
          return false;
        }
