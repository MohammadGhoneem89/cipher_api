...
module.exports = (req, res, next) => {
  ...
  try {
    ...
    if (!allowedMethods.includes(req.method))
      return sendError('method not allowed', 405, undefined, res);
      ...
      .then((allowed) => {
        ...
        if (commonConst.permissionExcludeList.indexOf(action) >= 0) {
          ...
          if (config.get('mode') === 'DEV') {
            sendError(`Do you want to add the permissions for action ${action}`, 200, undefined, res);
            // return next();
          } else {
            sendError('You are not allowed to access this resource', 403, undefined, res);
          }
...
function sendError(err, statusCode = 403, JBT, res) {
