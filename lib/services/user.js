  ...
  return validator.validate(payload, validator.schemas.user.userDetails)
    ...
    .then((res) => {
      ...
      for (const group of user.groups) {
        ...
        idList.push(group._id)
      }
      for (const group of groups) {
        if (idList.indexOf(group._id) == -1) {
...
function resetPassword(payload, msg, template, file) {
  ...
  return validator.validate(payload, validator.schemas.user.resetPassword)
    ...
    .then((user) => {
      ...
      if ((currtimestamp - user.lastResetTime) / 1000 > 30000) {
        console.log('here')
...
function createOnDemandWelCome(payloadRecv, errorMsg) {
  let payload = {
    api: {
  ...
  console.log(apiLnk);
  return APIDefination.getActiveAPIListForDocumentationNew().then((mapList) => {
    for (let usecase in mapList) {
      for (let api in mapList[usecase]) {
        let result = _.get(mapList, `${usecase}.${api}`, undefined)
        if (result) {
          let tuple = {
            "name": `${usecase} - ${api}`,
            "request": {
              ...
              "body": {
                "mode": "raw",
                "raw": JSON.stringify(result.requestSchema)
              },
              "url": `${config.get('webAddress')}/API/${usecase}/${api}`,
    ...
    let file = bone;
    return Promise.all([userRepo.create(payload.api), userRepo.create(payload.user)]).then((user) => {
      return resetPassword({
        action: 'resetPassword',
