...
function getDetails(payload) {
  return validator.validate(payload, validator.schemas.user.userDetails)
    .then(() => {
	  const query = payload.allowedGroups ? { name: { $in: payload.allowedGroups } } : {};
      return Promise.all([
...
customValidates = _.merge({}, customValidates, validator.schemas.user.userTypeAPI);
  ...
  return validator.errorValidate(payload, customValidates)
    .then(() => commonHelper.generatePassword())
    .then((policy) => {
...
payload.data.passwordHashType = policy.hashType;
    ...
    .then((validPass) => {
      ...
      payload.data.password = hash[payload.data.passwordHashType](payload.data.password);
      const params = {
    ...
    .then((user) => {
      if (_.get(payload, 'data.userType') === commonConst.user.userTypeKeys.human) {
        return resetPassword({ action: 'resetPassword', userID: user.userID, email: user.email }, msgConst.set);
      }
      return user;
    });
}
...
function updatePassword(payload) {
  ...
  return validator.validate(payload, validator.schemas.user.changePassword)
    ...
    .then((res) => {
      ...
      userDetails = res[1];
      try {
        newPassHash = hash[userDetails.passwordHashType](payload.data.newPassword);
      }
      catch (err) {
      ...
      if (hash[userDetails.passwordHashType](payload.data.oldPassword) !== userDetails.password) {
      ...
      for (const item of pDetails) {
        ...
        if (index && index.length && item.length) {
...
function authenticateUser(payload) {
  ...
  return validator.errorValidate(payload, validator.schemas.user.authenticateUser)
    ...
    .then((res) => {
      ...
      userDetails = res[1];
      if (!userDetails) {
        error = { userCode: 'User code is invalid' };
        throw error;
      }
      try {
        newPassHash = hash[userDetails.passwordHashType](payload.Password);
      }
      catch (err) {
        error = { desc: err.stack || err };
        throw error;
...
function resetPassword(payload, msg) {
  let userDetails;
  let passLink;
  return validator.validate(payload, validator.schemas.user.resetPassword)
    ...
    .then((user) => {
      userDetails = user;
      if (!user) {
        const error = { desc: 'Invalid userID or password' };
        throw error;
      }
    ...
    .then((link) => {
      passLink = link;
      return emailTemplateRepo.findAndFormat(msg.emailId, Object.assign(userDetails, link));
    })
    .then((template) => {
      const emailParams = {
      ...
      email.client(emailParams);
      const userObj = {
        passwordReset: passLink.passwordToken,
        updatedAt: dates.newDate(),
...
function setPassword(payload) {
  ...
  return validator.errorValidate(payload, validator.schemas.user.setPassword)
    .then(() => {
      const token = crypto.decrypt(payload.passwordToken);
      if (!token) {
    ...
    .then((res) => {
      ...
      const pDetails = [userDetails.userType, userDetails.orgType, userDetails.userID, userDetails.firstName, userDetails.lastName];
      for (const item of pDetails) {
        const index = payload.newPassword.toLocaleLowerCase().match(new RegExp(item.toLocaleLowerCase(), 'ig'));
        if (index && index.length) {
          error = { desc: 'Personal details not allowed' };
          throw error;
        }
  ...
  return validator.errorValidate(payload, validator.schemas.user.updateStatus)
    ...
    .then((user) => {
      ...
      if (user.isActive === payload.isActive) {
        ...
        error = { userCode: `User already ${activeMsg}` };
        throw error;
      }
      const userObj = {
...
function isAvailable(payload) {
  return validator.errorValidate(payload, validator.schemas.user.isAvailable)
    .then(() => {
      return userRepo.getGroups({ userID: payload.userCode });
    });
}
...
function logout(payload) {
  ...
  if (_.isEmpty(query)) {
    return Promise.reject({ token: 'Token not provided' });
  }
  return tokenLookupRepo.remove(query);
}

function unlock(payload) {
  return validator.errorValidate(payload, validator.schemas.user.unlockUser)
    .then(() => {
      const updateObj = {
      ...
      return userRepo.findOneAndUpdate({ userID: payload.userID }, updateObj);
    });
}
