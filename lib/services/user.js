...
async function approve(payload) {
  ...
  if (_.get(user, 'userType') == commonConst.user.userTypeKeys.human && _.get(user, 'isNewUser') == true) {
    console.log("here")   
   resetPassword({
  ...
  return validator.errorValidate(payload, validator.schemas.user.setPassword)
    .then(() => {
      ...
      if (payload.isAuth) {
        ...
        return Promise.all([
          commonHelper.validPassoword(payload.newPassword),
          userRepo.findOneWithPass({ _id: token._id })
        ])
      }
    ...
    .then((res) => {
      ...
      if (userDetails && validPass.valid) {
