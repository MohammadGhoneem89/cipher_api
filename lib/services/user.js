...
function getDetails(payload) {
  return validator.validate(payload, validator.schemas.user.userDetails)
    .then(() => {
      const query = payload.allowedGroups ? { name: { $in: payload.allowedGroups }, orgCode: payload.orgCode } : { orgCode: payload.orgCode };
...
function resetPassword(payload, msg) {
  ...
  return validator.validate(payload, validator.schemas.user.resetPassword)
    ...
    .then(async(link) => {
      ...
      console.log(global.config.email.authEmail);
      console.log(global.config.email.authPassword);
      console.log(global.config.email.authEmail);
      // console.log(link, 'LINK');
      // console.log(link, 'LINK');
      // console.log(link, 'LINK');
      // let decryptedPassword = _decrypt(global.config.email.authPassword);
      // console.log(decryptedPassword, '========---------0000000');
      console.log(global.config.email);
      // "enabled" : true,
      // "authEmail" : "3287275@gmail.com",
      // "authPassword" : "ac6961bf99d23318677d8343dbef7300",
      // "host" : "smtp.gmail.com",
      // "port" : 465,
      // "queue" : "EMAIL_QUEUE",
      // "ssl" : true
      let transporter = nodemailer.createTransport({
        ...
        secure: true, // use TLS
        auth: {
             user: 'zain.jawwad@avanzainnovations.com',//jahan se send 
             pass: 'Aisitaisi4696'//
         },
         tls: {
    ...
    console.log(global.config.email.address);
    console.log(userDetails.email);
    console.log(msg, 'MSG');
    let info = await transporter.sendMail({
      from: 'zain.jawwad@avanzainnovations.com', // sender address
      to: 'iqra.rauf@avanzainnovations.com',  // list of receivers
      subject: 'TEST',    // Subject line   
      html: `<p>${msg.para}<br><b><a href="${link.uri}">Click Here</a></b></p>`
  });
  console.log('Message sent: %s', info.messageId);
      ...
      // console.log(userDetails, 'USER DETAISSSSSSSSS')
      const userObj = {
