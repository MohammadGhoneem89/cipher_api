...
function findOne(payload) {
  return EmailTemplate.find(payload).lean().exec();
}
  ...
  return EmailTemplate
    ...
    .then((template) => {
      if (template) {
        ...
        //  console.log(">>checking >>>>"+replaceVal)
          ...
          var ctemplate = Handlebars.compile(template.templateTextEng);
         // var result = ctemplate(params);
          template.templateTextEng = ctemplate(params);
