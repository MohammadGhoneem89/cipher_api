    ...
    .then((template) => {
      if (template) {
        for (const prop in params) {
          template.templateTextEng = template.templateTextEng.replace(`{{${prop}}}`, params[prop]);
          template.templateTextArabic = template.templateTextArabic.replace(`{{${prop}}}`, params[prop]);
        }
        return template;
      }
      return false;
