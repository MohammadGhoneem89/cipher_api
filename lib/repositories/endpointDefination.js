...
function findAll(payload) {
  ...
  if (payload && payload.requestType) {
    query = {"requestType": payload.requestType};
  }
  return EndpointDefination.find(query).lean(true).exec();
}

function findPageAndCount(payload) {
  const query = {};
  if (_.get(payload, 'searchCriteria.address')) {
    query.address = {$regex: RegExp(payload.searchCriteria.address, 'gi'), $options: 'si'};
  }
  if (_.get(payload, 'searchCriteria.name')) {
