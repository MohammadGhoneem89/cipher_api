...
function getActiveAPIsData(payload) {
    ...
    query.useCase = payload.useCase || false;
    query.MSP = payload.orgTypes || false;
    console.log(JSON.stringify(query));
    let gte = {};
    let lte = {};
    return APIDefination
...
function findPageAndCount(payload) {
  ...
  return Promise.all([
    APIDefination
      ...
      .select('useCase  route documentPath isActive isSimulated createdBy createdAt isSmartContract MSP description')
