  ...
  return Promise.all([
    ...
    .then((res) => {
      ...
      if (!user) {
        ...
        error = {desc: 'username or password is incorrect'};
        throw error;
      }
    ...
    .then(() => {
      return {token: token, firstScreen: user.firstScreen, userType: user.userType, isNewUser: user.isNewUser};
    });
    ...
    if(user.userType == "API"){
      if (user.password !== (payload.password) || user.passwordRetries > policy.allowIncorrectLoginAttempts) {
        ...
        return userRepo.update({ _id: user._id }, data)
          .then(() => {
            insertAttempt({ userId: user._id, isValid: false });
            error = user.passwordRetries > policy.allowIncorrectLoginAttempts ? { desc: 'Your account is locked, password attempt exceeds the limit' , userType : user.userType } : { desc: 'username or password is incorrect' , userType : user.userType};
            throw error;
          });
      }
    } else {
