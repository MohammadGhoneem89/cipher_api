...
var processTrnx = function (trnxId, trnxPayLoad, callback) {
  ...
  switch (trnxId) {
    ...
    case "0001":
      QueryChain(trnxPayLoad, callback);
      break;
    default: {
      var response = {
        success: false,
...
var Querycallback = function (trnxPayLoad, Result, GRPCcallback) {
  ...
  if (Result.success == true) {
    logger.debug({
      fs: 'GRPCController.js',
      func: 'Querycallback'
    }, ' [Querycallback] query Chaincode Ended With Success!');
    var outMessage = CreateOutMessage(trnxPayLoad, Result);
    GRPCcallback(null, outMessage);
    logger.debug({
      fs: 'GRPCController.js',
      func: 'Querycallback'
    }, " [Querycallback] Operation Completed Successfully For Message : " + trnxPayLoad.Header.UUID);
  } else {
    logger.error({fs: 'GRPCController.js', func: 'Querycallback'}, ' [Querycallback] ' + Result.message);
    logger.error({
      fs: 'GRPCController.js',
      func: 'Querycallback'
    }, ' [Querycallback] query Chaincode Ended With Failure/Rejection!');
    var outMessage = CreateOutMessage(trnxPayLoad, Result);
    GRPCcallback(null, outMessage);
  }
}
...
function CreateOutMessage(trnxPayLoad, msg) {
  ...
  logger.debug({
    fs: 'GRPCController.js',
    func: 'CreateOutMessage'
  }, " [x] Sending Message Back to Caller!!! \n" + FormattedMessage);
