...
var routeConfiguration = require('../routeConfig/routeConfiguration.json');
var customFunctions = require('../Common/customFunctions.js');
var validationFunctions = require('../Common/validationFunctions.js');
...
var handleExternalRequest = function (payload, channel, incommingRoute, UUIDKey, responseCallback, JWToken, ConnMQ) {
  ...
  var ResponseCaller = function (data) {
    logger.debug({
      fs: 'RestController.js',
      func: 'ResponseCaller'
  ...
  var route = "";
  var ignoreAttrib = false;
  var isError = false;
  var errorField = "";
  try {

    var routeConfigTemp = routeConfiguration[channel][incommingRoute];
    ...
    var routeConfig = routeConfiguration[channel][route];

    var dependencyResponse = null;
    ...
    if (routeConfig.customMapping === true) {
      ...
      handleCustomMappingFunction(routeConfig.MappingfunctionName, routeConfig.CustomMappingFile, payload, UUIDKey, route, ResponseCaller, JWToken, routeConfig, ConnMQ);
      return;
    } else {
      logger.debug({
        fs: 'RestController.js',
    ...
    var mappingConfiguration = require(config.MappingConfig + routeConfig.mappingConfigFile);

    var tranMapping = mappingConfiguration[route];
    var RequestJSON = {};
    logger.debug({
      fs: 'RestController.js',
      func: 'handleExternalRequest'
    }, " [handleExternalRequest] Transforming data and performing validation checks");
    for (var i = 0; i < tranMapping.requestMapping.length; i++) {
      ignoreAttrib = false;
      var leftObjConfigurationMap = tranMapping.requestMapping[i][0];
      var rightObjConfigurationMap = tranMapping.requestMapping[i][1];
...
var getErrorMessage = function (field) {
  var message = field + ' field is missing or Invalid in the request'
  return message;
}
...
function parseAndMapResponse(response, dependencyResponse, tranMapping, ErrorMessage, UUIDKey) {
  ...
  for (var i = 0; i < tranMapping.responseMapping.length; i++) {
    ignoreAttrib = false;
    var leftObjConfigurationMap = tranMapping.responseMapping[i][0];
    var rightObjConfigurationMap = tranMapping.responseMapping[i][1];
...
function sendGRPCRequest(protoFile, srvcAddressNPort, Request, UUIDKey, responseCallback, tranMapping, ErrorMessage) {
  ...
  var RESTSrvc = "";
  var client = new RESTSrvc.BLA.BLAService(srvcAddressNPort, grpc.credentials.createInsecure());
  ...
  client.query(Request, function (error, msg) {
    ...
    if (error) {
      ...
      var resJS = parseAndMapResponse(msg, null, tranMapping, ErrorMessage, UUIDKey);
      logger.debug({
        fs: 'RestController.js',
        func: 'handleExternalRequest'
      }, " [handleExternalRequest] JSON: " + JSON.stringify(resJS, null, 2));
...
function handleCustomMappingFunction(MappingfunctionName, CustomMappingFile, payload, UUIDKey, route, callback, JWToken, routeConfig, MQConn) {
  logger.debug({
    fs: 'RestController.js',
    func: 'handleCustomMappingFunction'
  }, " [handleCustomMappingFunction] MappingfunctionName : " + MappingfunctionName);
  try {
    var fileLoc = config.CustomFunctionsLocation + CustomMappingFile;
    logger.debug({
      fs: 'RestController.js',
      func: 'handleCustomMappingFunction'
    }, " [handleCustomMappingFunction] Loading File : " + fileLoc);
    var mappingFunctions = require(fileLoc);
    logger.debug({
      fs: 'RestController.js',
      func: 'handleCustomMappingFunction'
    ...
    var outVal = mappingFunctions[MappingfunctionName](payload, UUIDKey, route, callback, JWToken, routeConfig, MQConn);
    logger.debug({
      fs: 'RestController.js',
      func: 'handleCustomMappingFunction'
...
function execTrigger(payload, extConfig) {
  ...
  var outVal = customFunctions[extConfig.functionName](payload);
  return outVal;
}
...
function execValidation(value, functionName) {
  ...
  var outVal = validationFunctions[functionName](value);
  return outVal;
}
