...
function refundBatchDetail(body) {
  ...
  if (body) {
    ...
    if (body.data.batchNo && body.data.batchNo !== '') {
      criteria.push({ 'data.RefundBatchID': body.data.batchNo });
    }
    if (body.data.entity && body.data.entity !== '') {
      criteria.push({ 'data.entity': body.data.entity });
    }
...
module.exports = refundBatchDetail;
