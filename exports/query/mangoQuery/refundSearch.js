...
function refundSearch(body) {

    logger.app.info(body, ' [ Refund Search ] Refund Search req : ');
    ...
    if (body) {
        ...
        if (body.type == 'refundList') {
            criteria.push({'data.RefundBatchID': ''});
        }
    }
    ...
    return Promise.all(promises)
        ...
        .catch((err) => {
	        logger.app.info(err, ' [ Refund Search ] Refund Search error : ');
        });
}

module.exports = refundSearch;
