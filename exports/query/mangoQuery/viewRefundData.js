...
function getRefundData(body) {
  ...
  if (body) {
    ...
    if (body.data.transactionID && body.data.transactionID !== '') {
      criteria.push({ 'data.RefundRef': body.data.transactionID });
    }
  ...
  return Promise.all(promises)
    ...
    .catch((err) => {
	    logger.app.error(err, ' [ Refund Data ] Refund Data error : ');
    });
}

module.exports = getRefundData;
