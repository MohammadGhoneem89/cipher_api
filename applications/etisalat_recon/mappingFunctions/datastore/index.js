...
function getDatastoreList(payload, UUIDKey, route, callback, JWToken) {
  let queryData = 'SELECT *, date_part(\'epoch\'::text,"createdAt")::bigint as "dateEpoch"  FROM datastore_elems WHERE 1=1 ';
    ...
    return Promise.all([
      ...
      data[1].rows.forEach((elemt) => {
        outVal.push({
          ...
          name: elemt.tranxData.key.replace(elemt.tranxData.dataStructureName + "_", ""),
      ...
      let response = {
        "getDatastoreDetail": {
          ...
          "data": {
            ...
            lat: _.get(attList, `EID_Y.attributesValue.GIS`, undefined),
            lng: _.get(attList, `EID_X.attributesValue.GIS`, undefined)
          }
        }
      };
      return callback(response);
...
function getEidList(payload, UUIDKey, route, callback, JWToken) {
  let queryData = `SELECT *, date_part('epoch'::text,"createdAt")::bigint as "dateEpoch"  FROM datastore_elems WHERE 1=1 AND "tranxData"->>'dataStructureName' = 'EID'`;
  let queryCnt = `SELECT count(*)  FROM datastore_elems WHERE 1=1 AND "tranxData"->>'dataStructureName' = 'EID'`;
  let query = '';
  if (payload.searchCriteria && payload.searchCriteria.toDate && payload.searchCriteria.fromDate) {
    let fromdate = dates.ddMMyyyyslash(payload.searchCriteria.fromDate);
    let todate = dates.ddMMyyyyslash(payload.searchCriteria.toDate);
    ...
    return Promise.all([
      ...
      data[1].rows.forEach((elemt) => {
        ...
        outVal.push({
          ...
          key: elemt.tranxData.key,
          name: elemt.tranxData.key.replace(elemt.tranxData.dataStructureName + "_", ""),
          type: _.get(elemt, 'tranxData.attributeList.FDH_TYPE.attributesValue.GIS', '-'),
          region: _.get(elemt, 'tranxData.attributeList.REGION_CODE.attributesValue.GIS', '-'),
          status: _.get(elemt, 'tranxData.attributeList.EID_Status.attributesValue.GIS', '-'),
          reconStatus: _.get(elemt, 'tranxData.attributeList.EID_Status.isReconciled', false) ? "Reconciled" : "Not Reconciled",
          correction: _.get(elemt, 'correction', ''),
          keyAttrValue: elemt.tranxData.keyAttrValue,
          createdon: elemt.createdAt,
          dateEpoch: elemt.dateEpoch * 1000,
          updatedon: elemt.updatedAt
