...
exports.updateTask = function (payload, UUIDKey, route, callback, JWToken) {
    ...
    let commentInsertQuery = 'INSERT INTO taskcomments (commentdate, username, commenttext, type ,taskId, ecd, reason, attributeid, attributename, status) values '// + payload.comments.commentdate + '\', \'' + payload.comments.username  + '\', \'' + payload.comments.commenttext + '\', \'' + payload.comments.type + '\', \'' + payload.taskId  + '\', \'' + payload.comments.ecd  + '\', \'' + payload.comments.reason  + '\', \'' + payload.comments.attributeid  + '\')';
    ...
    pg.connection().then((conn) => {
        return Promise.all([
            ...
            console.log(result)
            let response = { "responseMessage": { "action": "updateTask", "data": { "message": { "status": "OK", "errorDescription": "Task Updated Success!!", "displayToUser": true, "newPageURL": "/hyperledger/workboard" } } } }
