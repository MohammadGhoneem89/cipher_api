...
func (t *URChainCode) postDataToBlockchainREGAUTH(stub hypConnect, args []string, functionName string, orgType string) pb.Response {
	...
	if unifiedRegDataAsBytes == nil {
		...
		var oldUnifiedRegData_AsJSON = unifiedRegDataAsJSON
		oldUnifiedRegData_AsJSON.Key = unifiedRegDataAsJSON.Key + "_OLD"

		//marshal old document
		oldunifiedRegJSONasBytes, err := json.Marshal(oldUnifiedRegData_AsJSON)
		if err != nil {
			return shim.Error(err.Error())
		}

		// insert old document
		err = insertData(&stub, oldUnifiedRegData_AsJSON.Key, "unifiedReg_"+s[0], []byte(oldunifiedRegJSONasBytes))
		if err != nil {
			return shim.Error(err.Error())
		}
		//=================================================================================================================
